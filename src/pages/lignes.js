...
export class LignesPage extends HTMLElement {
    ...
    scanLotModal(detail) {
        ...
        let html = `
            <div class='w-[500px]'>
                <h3 class='font-bold text-3xl'>Scanner un lot</h3>
                <div class='relative'>
                    <sg-input input='number' idname='inpScanLot' placeholder='Numéro de lot'></sg-input>
                    <div class='absolute right-6'><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-scan-barcode-icon lucide-scan-barcode"><path d="M3 7V5a2 2 0 0 1 2-2h2"/><path d="M17 3h2a2 2 0 0 1 2 2v2"/><path d="M21 17v2a2 2 0 0 1-2 2h-2"/><path d="M7 21H5a2 2 0 0 1-2-2v-2"/><path d="M8 7v10"/><path d="M12 7v10"/><path d="M17 7v10"/></svg></div>
                </div>
            </div>
            <div class='w-[500px]'>
                <sg-btn idname='saisieManuelle' css='bg-dblueBase text-white rounded-md w-full'>Saisie Manuelle</sg-btn>
            </div>
        `;
        document.getElementById('modalContent').innerHTML = html;

        document.getElementById('inpScanLot').addEventListener('input', (event) => {
            handleScan(event, JSON.stringify(detail));
        });
            ...
            <main id='main' role='main' class='h-full flex flex-col relative'>
                <div id='entete' class='flex flex-col items-center w-full absolute top-0 left-0 right-0 z-10 -translate-y-[6.75rem]'>
                    <div class='bg-white shadow p-3 w-full flex flex-col gap-3'>
                        ...
                        <div class='flex justify-between'>
                            <sg-input idname='dateFiltre' label='Tournée' input='date' inputCss='grow bg-gray-200 opacity-60' class='w-[45%]' disabled='true'></sg-input>
                        </div>
                    </div>
